...
func RunLeaderRole(ctx context.Context, asg *models.Assignment, sess adapters.SessionWrapper) error {
	...
	log.Printf("[Leader] Node %s is now acting as cluster leader for assignment %s. Lease ID: %d", asg.NodeID, asg.ID, sess.LeaseID())
	...
	for {
		...
		case <-ticker.C:
			...
			/*
			   ...
			   - renew assignment leases (dynamic runtime call to sess.LeaseID())
