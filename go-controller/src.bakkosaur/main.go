...
func main() {
	...
	go roles.RunMemberRole(ctx, s, config.NodeID())

	go func() {
		log.Printf("[Main] Starting assignment watch loop for node: %s", config.NodeID())

		var lastRevision int64

		initialIDs, rev, err := s.GetNodeAssignmentsWithRev(ctx, config.NodeID())
		if err != nil {
		...
		for {
			...
			case <-ctx.Done():
				return
			default:
				watchChan := s.WatchNodeAssignmentsFromRev(ctx, config.NodeID(), lastRevision+1)
